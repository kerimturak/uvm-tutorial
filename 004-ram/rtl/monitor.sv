class monitor extends uvm_monitor;
  ...
  function void connect_phase(uvm_phase phase);
    if (!uvm_config_db#(virtual ram_if)::get(this, "", "vif", vif)) `uvm_error("NOVIF", {"virtual interface must be set for: ", get_full_name(), ".vif"})
  endfunction
  ...
  task run_phase(uvm_phase phase);
    ...
    forever begin
      ...
      if (!vif.rw_en_i) begin
        ...
        `uvm_info(get_type_name(), $sformatf("Monitor: Read completed - addr: %0d, data_o: %0d", mon_packet.addr_i, mon_packet.data_o), UVM_MEDIUM);
      end else begin
        `uvm_info(get_type_name(), $sformatf("Monitor: Write operation - addr: %0d, data_i: %0d", mon_packet.addr_i, mon_packet.data_i), UVM_MEDIUM);
      end
