class scoreboard extends uvm_scoreboard;
  ...
  virtual function void write(packet item);
    `uvm_info(get_type_name(), $sformatf("Scoreboard: Received transaction - addr: %0d, data_i: %0d, rw_en: %0d, data_o: %0d",
                                         item.addr_i, item.data_i, item.rw_en_i, item.data_o), UVM_MEDIUM);
    
    if (item.rw_en_i) begin
      // Yazma işlemi -> Veriyi RAM modeli olan `trans[]` dizisine kaydet
      trans[item.addr_i].data_i = item.data_i;
      `uvm_info(get_type_name(), $sformatf("Scoreboard: Write to addr %0d, data %0d", 
                                           item.addr_i, item.data_i), UVM_MEDIUM);
    end else begin
      // Okuma işlemi -> Beklenen veri ile karşılaştırma yap
      if (trans[item.addr_i].data_i == item.data_o) begin 
        `uvm_info(get_type_name(), $sformatf("PASS: Read addr %0d, expected %0d, got %0d", 
                                             item.addr_i, trans[item.addr_i].data_i, item.data_o), UVM_MEDIUM);
      end else begin
        `uvm_error(get_type_name(), $sformatf("FAIL: Read addr %0d, expected %0d, got %0d", 
                                              item.addr_i, trans[item.addr_i].data_i, item.data_o));
        fail_count++;
      end
    end
  endfunction
